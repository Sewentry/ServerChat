...
public class ClientHandler {
    ...
    private void authentication() throws IOException {
          while (true){
              ...
              if(message.startsWith(AUTH_CMD_PREFIX)){
                  boolean isSucsessAuth = processAuthentication(message);
                  if(isSucsessAuth){
                      break;
                  }
              }else{
    ...
    private boolean processAuthentication(String message) throws IOException {
        ...
        if(username!=null){
            ...
            out.writeUTF(AUTHERR_CMD_PREFIX+" Логин или пароль не найдены");
            return false;
        }
    ...
    public void sendPrivateMessage(String sender, String message) throws IOException {
        out.writeUTF(String.format("%s %s %s",sender,": ",message));
    }
}
